...
interface ProgressConstellationProps {
  user: User;
  quizzes: Quiz[];
  onStarClick: (details: { title: string; data: any[] }) => void;
}
...
export function ProgressConstellation({ user, quizzes, onStarClick }: ProgressConstellationProps) {
  ...
  const connections = [
   ...
   const handleStarClick = (star: StarData) => {
    if (star.id === 'quizzes') {
        onStarClick({ title: 'Perfected Quizzes', data: quizzes.map(q => ({ title: q.title })) });
    } else {
         onStarClick({ title: star.label, data: [] });
    }
  };
