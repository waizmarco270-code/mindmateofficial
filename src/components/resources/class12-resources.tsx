...
const UNLOCK_PASSWORD = "waizextraclass12";
const UNLOCK_CREDITS = 30;

export function Class12Resources() {
    const { class12Resources, loading } = useResources();
    const { user } = useAuth();
    ...
    const isPremiumUnlocked = currentUserData?.class12Unlocked ?? false;
    const currentCredits = currentUserData?.credits ?? 0;

    const handleUnlockWithPassword = async () => {
        if (!user) return;
        if (password === UNLOCK_PASSWORD) {
            await unlockResourceSection(user.uid, 'class12');
            setIsUnlockDialogOpen(false);
            toast({ title: 'Success!', description: 'Class 12 content unlocked.' });
        } else {
    ...
    const handleUnlockWithCredits = async () => {
        if (user && currentCredits >= UNLOCK_CREDITS) {
            await unlockResourceSection(user.uid, 'class12', UNLOCK_CREDITS);
            setIsUnlockDialogOpen(false);
            ...
            <>
                <Card className="border-primary/20 bg-primary/5 cursor-pointer hover:border-primary/40 transition-all" onClick={handleOpenDialog}>
                    <CardHeader className="flex flex-row items-center gap-4">
                        ...
                        <div>
                            <CardTitle>CLASS 12 BOARDS PREMIUM</CardTitle>
                ...
                <Dialog open={isUnlockDialogOpen} onOpenChange={setIsUnlockDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle className="flex items-center gap-2"><Unlock className="h-5 w-5" /> Unlock Class 12 Content</DialogTitle>
                            <DialogDescription>
                                Gain lifetime access to all Class 12 premium study materials.
                            </DialogDescription>
                        </DialogHeader>
                            ...
                            <div className="space-y-4 rounded-lg border p-4">
                                <h3 className="font-semibold flex items-center gap-2"><KeyRound className="h-4 w-4"/> Unlock with Password</h3>
                                 <div className="space-y-2">
                                    <Label htmlFor="password-class12">Password</Label>
                                    <Input id="password-class12" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter unlock password" />
                                </div>
    ...
    return (
        <div className="space-y-4">
             <div>
                 <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2 text-primary"><Unlock /> CLASS 12 BOARDS PREMIUM</h2>
                <p className="text-muted-foreground">Exclusive unlocked content. Keep up the great work!</p>
            </div>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                 ...
                 {!loading && class12Resources.map(resource => (
                 ...
                 {!loading && class12Resources.length === 0 && (
                    <p className="text-muted-foreground col-span-3 text-center">No Class 12 resources have been added yet. Check back later!</p>
                )}
            </div>
        </div>
    );
