    ...
    return (
        <AnimatePresence>
            {isVisible && activeGlobalGift && (
                <motion.div initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -20 }} className="mb-8">
                    <Card className="relative overflow-hidden border-2 border-yellow-400/30 bg-slate-900 shadow-[0_0_50px_rgba(250,204,21,0.15)] rounded-[2rem]">
                        ...
                        <CardContent className="relative p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6">
                             <div className="relative">
                                <motion.div animate={{ rotate: 360 }} transition={{ duration: 10, repeat: Infinity, ease: 'linear' }} className="absolute -inset-4 border border-dashed border-yellow-400/20 rounded-full"/>
                                <div className="relative p-5 rounded-3xl bg-yellow-400/10 border-2 border-yellow-400/30">
                            ...
                            <div className="flex-1 text-center sm:text-left min-w-0">
                                ...
                                <p className="text-base text-slate-300 font-medium italic">"{activeGlobalGift.message}"</p>
                                {renderRewardsPreview()}
                            </div>
                            <div className="flex flex-col items-center gap-2 w-full sm:w-auto">
                                <Button onClick={handleClaim} disabled={isClaiming} className={cn("h-16 px-8 rounded-2xl bg-yellow-400 text-black font-black text-lg shadow-xl")}>
                                    {isClaiming ? <Loader2 className="animate-spin h-6 w-6" /> : <><Sparkles className="mr-2 h-5 w-5" /> SECURE ASSETS</>}
                                </Button>
                                {activeGlobalGift.maxClaims && (
                                    <p className="text-[10px] font-black uppercase text-yellow-400/60">Limit: {activeGlobalGift.claimedBy?.length || 0} / {activeGlobalGift.maxClaims}</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
...
function RewardPill({ icon: Icon, text, color }: { icon: any, text: string, color: string }) {
    return (
        <div className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/40 border border-white/5", color)}>
