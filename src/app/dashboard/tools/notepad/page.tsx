...
export default function NotepadPage() {
    ...
    const lineCount = useMemo(() => notes.trim() === '' ? 0 : notes.trim().split('\n').length, [notes]);
    const paragraphCount = useMemo(() => notes.trim() === '' ? 0 : notes.trim().split(/\n\s*\n/).filter(p => p.trim() !== '').length, [notes]);
    const sentenceCount = useMemo(() => notes.trim() === '' ? 0 : (notes.match(/[.!?…]+(\s|$)/g) || []).length, [notes]);
    ...
    const handleReplace = () => {
        if (!findText) {
            toast({ variant: 'destructive', title: "Find field cannot be empty." });
            return;
        }
        const newNotes = notes.replaceAll(findText, replaceText);
        setNotes(newNotes);
        setSaveStatus('saving');
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm"><CaseUpper className="mr-2"/> Convert</Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent>
                        ...
                        <DropdownMenu>
                             <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm"><Wand2 className="mr-2"/> Tools</Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent>
                                ...
                                <DropdownMenuItem onClick={handleSortLines}><SortAsc className="mr-2"/> Sort Lines (A-Z)</DropdownMenuItem>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Input id="find-text" value={findText} onChange={(e) => setFindText(e.target.value)} placeholder="Text to find..."/>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="replace-text">Replace with</Label>
                            <Input id="replace-text" value={replaceText} onChange={(e) => setReplaceText(e.target.value)} placeholder="Text to replace with..."/>
                        </div>
                    </div>
                    <DialogFooter>
                        <DialogClose asChild>
