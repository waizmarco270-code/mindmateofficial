...
import { ArrowLeft, ArrowRight, Sparkles, AlertTriangle, ShieldCheck, Trophy } from 'lucide-react';
import { useChallenges, type DailyGoal } from '@/hooks/use-challenges';
...
export default function CreateChallengePage() {
    ...
    const { entryFee, reward } = useMemo(() => {
        let difficulty = 0;
        difficulty += duration * 1; // 1 point per day
        difficulty += studyHours * 2; // 2 points per hour of study
        difficulty += focusSessions * 5; // 5 points per focus session
        if (completeTasks) difficulty += 20; // 20 points for task completion goal
        
        const baseFee = 20;
        const baseReward = 50;

        const calculatedFee = Math.round(baseFee + (difficulty * 0.5));
        const calculatedReward = Math.round(baseReward + (difficulty * 1.5));
    ...
    const handleStartChallenge = async () => {
        ...
        dailyGoals.push({ id: 'checkIn', description: 'Check-in before midnight', target: 1 });
        ...
        await startChallenge({
            ...
            dailyGoals,
            rules: ["You must complete all your custom daily goals before midnight each day.", "Failure to complete any goal means the challenge is failed.", "The entry fee is refunded along with the reward upon successful completion."],
            eliteBadgeDays: 0, // No elite badge for custom challenges for now
        });
                            ...
                            {step === 1 && (
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <Slider value={[duration]} onValueChange={(v) => setDuration(v[0])} min={7} max={30} step={1} />
                                    </div>
                                </div>
                            )}
                             {step === 2 && (
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <Slider value={[studyHours]} onValueChange={(v) => setStudyHours(v[0])} min={0} max={10} step={1} />
                                    </div>
                                     <div className="space-y-3">
                                    ...
                                    <div className="flex items-center space-x-2">
                                     ...
                                     <div className="p-3 rounded-lg bg-muted/50 text-xs text-muted-foreground flex items-center gap-2">
                                        ...
                                        <span>A 'Daily Check-in' goal is automatically included.</span>
                                    </div>
                                </div>
                            )}
                             {step === 3 && (
                                     ...
                                     <Card className="text-left">
                                        ...
                                        <CardContent>
                                            <ul className="list-disc pl-5 space-y-2 text-sm text-muted-foreground">
                                                {studyHours > 0 && <li>Study for at least {studyHours} hours daily.</li>}
                                                {focusSessions > 0 && <li>Complete {focusSessions} focus session(s) daily.</li>}
                            ...
                            <AlertDialogTrigger asChild>
                             ...
                             <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction onClick={handleStartChallenge}>Yes, Start My Challenge</AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
                        </AlertDialog>
                    )}
